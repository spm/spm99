        ...
        for i = spm_input(str,'+1','e',1,...
                ...
                switch Ptype
                        ...
                        p     = spm_input(str,'+1','r',[],[ns,1],...
                                'batch',{'parametrics',s},'parameters',sillyvar);
...
function sf = make_sf(nscans,ntb,ons,durs,vals)
% make stick function
if nargin < 5
  vals = ones(size(ons));
end
if any(diff([prod(size(ons)), prod(size(durs)), prod(size(vals))]))
  error('Need same no of onsets, durations, parametric values');
end
ntot = nscans*ntb;
ons_st   = sparse(ntot,1);
for p = 1:length(ons)
  q  = round(ons(p)*ntb + 1);
  ons_st(q:(q + durs(p))) = vals(p);
end
sf = ons_st(1:ntot);
return
