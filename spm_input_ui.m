function varargout = spm_input_ui(varargin)
...
%
% spm_input_ui uses the SPM 'Interactive' window, which is 'Tag'ged
...
%
%-----------------------------------------------------------------------
% Programers help is contained in the main body of spm_input_ui.m
%-----------------------------------------------------------------------
...
%_______________________________________________________________________
% @(#)spm_input_ui.m	2.46 Andrew Holmes 01/03/14

%=======================================================================
% - FORMAT specifications for programers
%=======================================================================
% generic    - [p,YPos] = spm_input_ui(Prompt,YPos,Type,...)
% string     - [p,YPos] = spm_input_ui(Prompt,YPos,'s',DefStr)
% string+    - [p,YPos] = spm_input_ui(Prompt,YPos,'s+',DefStr)
% evaluated  - [p,YPos] = spm_input_ui(Prompt,YPos,'e',DefStr,n)
% - natural  - [p,YPos] = spm_input_ui(Prompt,YPos,'n',DefStr,n,mx)
% - whole    - [p,YPos] = spm_input_ui(Prompt,YPos,'w',DefStr,n,mx)
% - integer  - [p,YPos] = spm_input_ui(Prompt,YPos,'i',DefStr,n)
% - real     - [p,YPos] = spm_input_ui(Prompt,YPos,'r',DefStr,n,mm)
% condition  - [p,YPos] = spm_input_ui(Prompt,YPos,'c',DefStr,n,m)
% contrast   - [p,YPos] = spm_input_ui(Prompt,YPos,'x',DefStr,n,X)
% permutation- [p,YPos] = spm_input_ui(Prompt,YPos,'p',DefStr,P,n)
% button     - [p,YPos] = spm_input_ui(Prompt,YPos,'b',Labels,Values,DefItem)
% button/edit combo's (edit for string or typed scalar evaluated input)
%              [p,YPos] = spm_input_ui(Prompt,YPos,'b?1',Labels,Values,DefStr,mx)
%   ...where ? in b?1 specifies edit widget type as with string & eval'd input
%            - [p,YPos] = spm_input_ui(Prompt,YPos,'n1',DefStr,mx)
%            - [p,YPos] = spm_input_ui(Prompt,YPos,'w1',DefStr,mx)
% button dialog 
%            - [p,YPos] = spm_input_ui(Prompt,YPos,'bd',...
%                                Labels,Values,DefItem,Title)
% menu       - [p,YPos] = spm_input_ui(Prompt,YPos,'m',Labels,Values,DefItem)
% display    -            spm_input_ui(Message,YPos,'d',Label)
% display    - (GUI only) spm_input_ui(Alert,YPos,'d!',Label)
%
% yes/no     - [p,YPos] = spm_input_ui(Prompt,YPos,'y/n',Values,DefItem)
% buttons (shortcut) where Labels is a bar delimited string
%            - [p,YPos] = spm_input_ui(Prompt,YPos,Labels,Values,DefItem)
%
...
% WINDOWS:
%
% spm_input_ui uses the SPM 'Interactive' 'Tag'ged window. If this isn't
...
%
% Yes/No question shortcut - p = spm_input_ui(Prompt,YPos,'y/n') expands
% to p = spm_input_ui(Prompt,YPos,'b','yes|no',...), enabling easy use of
% spm_input_ui for yes/no dialogue. Values defaults to 'yn', so 'y' or 'n'
...
%               'a' & 's' (or 'A' & 'S').
%       p = spm_input_ui(str,-4,'b','AnCova|Scaling',[],2)
%               As above, but makes "Scaling" the default response, and
%               overrides global CMDLINE
%       p = spm_input_ui(str,0,'b','AnCova|Scaling|None',[1,2,3])
...
%               insensitive).
%       p = spm_input_ui(str,1,'b','AnCova',1)
...
%	p = spm_input_ui(str,'+0','br1','None|Mask',[-Inf,NaN],0.8)
...
%       p = spm_input_ui(str,'!0','m','Single Subject|Multi Subject|Multi Study')
...
%
%-----------------------------------------------------------------------
% FORMAT h = spm_input_ui(Prompt,YPos,'m!',Labels,cb,UD,XCB);
% GUI PullDown menu utility - creates a pulldown menu in the Interactive window
% FORMAT H = spm_input_ui(Prompt,YPos,'b!',Labels,cb,UD,XCB);
...
% conveniences of the spm_input_ui 'm'enu & 'b'utton types. If Prompt is
...
% structure, and the PopUp's callback is set to spm_input_ui('!m_cb'),
...
% msg     - status message
%
% FORMAT hM = spm_input_ui('!InptConMen',Finter,H)
...
%
% FORMAT [PLoc,cF] = spm_input_ui('!PointerJumpBack',PLoc,cF)
% Replace pointer and reset CurrentFigure back
% PLoc  - Pointer location before jumping
% cF    - Previous current figure
%
% FORMAT spm_input_ui('!PrntPrmpt',Prompt,TipStr,Title)
% Print prompt for CmdLine questioning
% Prompt - prompt string, callstr, or string matrix
% TipStr - tip string
% Title  - title string
%
% FORMAT [Frec,QRec,PRec,RRec] = spm_input_ui('!InputRects',YPos,rec,F)
...
%
% FORMAT spm_input_ui('!DeleteInputObj',F)
% Deltes input objects (only) from figure F
% F     - Interactive Figure, Defaults to spm_figure('FindWin','Interactive')
%
% FORMAT [CPos,hCPos] = spm_input_ui('!CurrentPos',F)
...
% NPos    - Next position index
%
% FORMAT MPos = spm_input_ui('!MaxPos',F,FRec3)
...
% 
% FORMAT spm_input_ui('!EditableKeyPressFcn',h,ch)
% KeyPress callback for GUI string / eval input
%
% FORMAT spm_input_ui('!ButtonKeyPressFcn',h,Keys,DefItem,ch)
% KeyPress callback for GUI buttons
%
% FORMAT spm_input_ui('!PullDownKeyPressFcn',h,ch,DefItem)
% KeyPress callback for GUI pulldown menus
%
% FORMAT spm_input_ui('!m_cb')
% Extended CallBack handler for 'p' PullDown utility type
%
% FORMAT spm_input_ui('!dScroll',h,str)
...
if ~isempty(Prompt) & ischar(Prompt) & Prompt(1)=='!'
	...
	[CmdLine,YPos] = spm_input_ui('!CmdLine',YPos);

	if ~CmdLine	%-Setup for GUI use
		%-Locate (or create) figure to work in
		Finter = spm_input_ui('!GetWin');
	
		%-Find out which Y-position to use, setup for use
		YPos = spm_input_ui('!SetNextPos',YPos,Finter,CmdLine);
	
		%-Determine position of objects
		[FRec,QRec,PRec,RRec]=spm_input_ui('!InputRects',YPos,'',Finter);
	end
end
...
%-----------------------------------------------------------------------
	spm_input_ui('!PrntPrmpt',[Prompt,strN,strM],TTstr)
...
%-----------------------------------------------------------------------
	...
	hM = spm_input_ui('!InptConMen',Finter,[hPrmpt,hDef,h]);
	...
	spm_input_ui('!PointerJumpBack',PLoc,cF)
	drawnow
...
%=======================================================================
	...
	if CmdLine
		...
		spm_input_ui('!PrntPrmpt',Prompt,'',Title)
		%-Build prompt
		%-------------------------------------------------------
		if ~isempty(Labs) 
...
if CmdLine
	...
	%-Print banner prompt
	%---------------------------------------------------------------
	spm_input_ui('!PrntPrmpt',Prompt)		%-Display question prompt
	...
	%-Bring window to fore & jump pointer to default button
	[PLoc,cF] = spm_input_ui('!PointerJump',RRec,Finter,RRec(3)*0.95);
...
%=======================================================================
	...
	if CmdLine
		...
		if nLabels==1
			...
			%-Figure ContextMenu for shortcuts
			hM = spm_input_ui('!InptConMen',Finter,[hPopUp,H]);

			%-Bring window to fore & jump pointer to menu widget
			[PLoc,cF] = spm_input_ui('!PointerJump',RRec,Finter);
	
			%-Callback for KeyPresses
			cb=['spm_input_ui(''!PullDownKeyPressFcn'',',...
				'findobj(gcf,''Tag'',''',Tag,'''),',...
...
%=======================================================================
% H = spm_input_ui(Prompt,YPos,'p',Labels,cb,UD,XCB)
%-Condition arguments
...
%-Sort out UserData for extended callbacks (handled by spm_input_ui('!m_cb')
%-----------------------------------------------------------------------
...
switch lower(Type), case 'm!'
	if XCB, UD.cb=cb; cb = {'spm_input_ui(''!m_cb'')'}; end
...
case 'b!'
	...
	for i=1:nLabels
		...
		if XCB, UD.cb=tcb; tcb = {'spm_input_ui(''!m_cb'')'}; end
		h = uicontrol(Finter,'Style','Pushbutton',...
			'String',deblank(Labels(i,:)),...
			'ToolTipString','',...
			'Tag',Tag,...
	...
	if length(Prompt)>56
		...
		uicontrol(Finter,'Style','PushButton','String','>',...
			...
			'CallBack',[...
			 ...
			 'spm_input_ui(''!dScroll'',get(gcbo,''UserData'')),',...
			 'set(gcbo,''Visible'',''on'')'],...
...
case '!colour'
%=======================================================================
% colour = spm_input_ui('!Colour')
varargout = {COLOUR};


case '!icond'
%=======================================================================
% [iCond,msg] = spm_input_ui('!iCond',str,n,m)
...
if ischar(i)
	if i(1)=='0' & all(ismember(unique(i(:)),setstr(abs('0'):abs('9'))))
		...
		i = [zeros(1,z), spm_input_ui('!iCond',i(z+1:end))'];
	else
...
case '!pointerjump'
%=======================================================================
% [PLoc,cF] = spm_input_ui('!PointerJump',RRec,F,XDisp)
%-Raise window & jump pointer over question
...
case '!prntprmpt'
%=======================================================================
% spm_input_ui('!PrntPrmpt',Prompt,TipStr,Title)
%-Print prompt for CmdLine questioning
...
case '!inputrects'
%=======================================================================
% [Frec,QRec,PRec,RRec,Sz,Se] = spm_input_ui('!InputRects',YPos,rec,F)
...
case '!deleteinputobj'
%=======================================================================
% spm_input_ui('!DeleteInputObj',F)
if nargin<2, F='Interactive'; else, F=varargin{2}; end
h = spm_input_ui('!FindInputObj',F);
delete(h(h>0))


case {'!currentpos','!findinputobj'}
%=======================================================================
% [CPos,hCPos] = spm_input_ui('!CurrentPos',F)
% h            = spm_input_ui('!FindInputObj',F)
...
case '!nextpos'
%=======================================================================
% [NPos,CPos,hCPos] = spm_input_ui('!NextPos',YPos,F,CmdLine)
%-Return next position to use
if nargin<3, F='Interactive'; else, F=varargin{3}; end
if nargin<2, YPos='+1'; else, YPos=varargin{2}; end
if nargin<4, [CmdLine,YPos]=spm_input_ui('!CmdLine',YPos);
...
if nargout<3
	CPos = spm_input_ui('!CurrentPos',F);
	hCPos = [];
else
	[CPos,hCPos] = spm_input_ui('!CurrentPos',F);
end
...
case '!setnextpos'
%=======================================================================
% NPos = spm_input_ui('!SetNextPos',YPos,F,CmdLine)
...
case '!maxpos'
%=======================================================================
% MPos = spm_input_ui('!MaxPos',F,FRec3)
%
if nargin<3
	if nargin<2, F='Interactive'; else, F=varargin{2}; end
	F = spm_figure('FindWin',F);
...
%=======================================================================
% spm_input_ui('!EditableKeyPressFcn',h,ch,hPrmpt)
...
%=======================================================================
% spm_input_ui('!ButtonKeyPressFcn',h,Keys,DefItem,ch)
...
%=======================================================================
% spm_input_ui('!PullDownKeyPressFcn',h,ch,DefItem)
if nargin<2, error('Insufficient arguments'), else, h=varargin{2}; end
...
if ~(isfield(tmp,'cb') & isfield(tmp,'UD'))
	error('Invalid UserData structure for spm_input_ui extended callback')
end
cb  = tmp.cb;
UD  = tmp.UD;
...
case 'c'
	...
	[p,msg] = spm_input_ui('!iCond',str,n,m);
case 'x'
